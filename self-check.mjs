...
Mix dry ingredients.`);
  ...
  assert(timed.includes("[Step 1] Bake, ~{35 min}"), `timer: ${timed.match(/\[Step 1\].*/)?.[0]}`);
  assert(timed.includes("177 °C"), `temp: ${timed.match(/\[Step 2\].*/)?.[0]}`);
  assert(timed.includes("[Step 3] Chill the dough, ~{120 min}"), `hours timer: ${timed.match(/\[Step 3\].*/)?.[0]}`);
  assert(timed.includes("~{30 s}"), `seconds timer: ${timed.match(/\[Step 4\].*/)?.[0]}`);

  // new unit mappings
  assert(toGramQty("1", "gal") === "3800 ml", `gal ${toGramQty("1", "gal")}`);
  assert(toGramQty("2", "sticks") === "226 g", `stick ${toGramQty("2", "sticks")}`);
